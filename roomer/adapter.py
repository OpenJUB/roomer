...
class DreamjubAdapter(DefaultSocialAccountAdapter):
    ...
    def populate_user(self, request, sociallogin, data):
        ...
        if extra['status'] == 'foundation-year' or extra['status'] == 'medprep':
            ...
            user.housing_type = max(6, min(7, abs(extra['year'] - year_now) + 6 + 2)) # settings.HOUSING_TYPE_MS_(1|2)
        elif extra['status'] == 'undergrad':
            user.housing_type = max(3, min(5, abs(extra['year'] - year_now) + 3 + 2)) # settings.HOUSING_TYPE_UG_(1|2|3)
